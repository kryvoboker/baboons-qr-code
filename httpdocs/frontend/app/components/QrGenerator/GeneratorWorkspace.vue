...
onMounted(() => {
    ...
    if (pending) {
        ...
        if (template) {
            try {
                const parsed = JSON.parse(template) as { kind?: typeof draft.value.kind; design?: typeof draft.value.design; name?: string };
...
const save = async () => {
    ...
    try {
        const response = await $fetch<{ data: { id: string } }>('/api/bff/v1/qr-codes', {
            method: 'POST',
            body: {
...
<template>
    <section id="generator" class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div class="mb-6">
